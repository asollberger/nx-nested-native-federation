...
async function loadRemoteModuleAfterManifest<T = any>(
    remoteName: string,
    ...
    await (manifestLoaded
        ...
        : fetch('/assets/federation.manifest.json')
              ...
              .then((manifestRemotes: Record<string, string>) =>
                  Promise.allSettled(
                      Object.entries(manifestRemotes).map(
                          ([federationRemoteName, federationInfoUrl]) =>
                              processRemoteInfo(
                                  federationInfoUrl,
                                  federationRemoteName
                              )
                      )
                  )
              )
        ...
        children: [
            {
                path: 'accounts',
                loadChildren: () =>
                    loadRemoteModuleAfterManifest('accounts', './routes').then(
                        (m) => m.appRoutes
                    ),
            },
            {
                path: 'clients',
                loadChildren: () =>
                    loadRemoteModuleAfterManifest('clients', './routes').then(
                        (m) => m.appRoutes
                    ),
            },
        ],
